...
The wrapper owns a mutex-protected ordinary configuration containing the API base, selected guild,
Discord delivery mode, and optional channel ID. Initial and updated values must pass the same
structural HTTPS and positive-decimal ID rules used at the settings boundary. An enabled DonBot
configuration requires a selected verified guild.
...
The captured delivery mode is none, guild defaults, or channel override. Channel override requires a
canonical channel ID. An explicit user Reupload always captures none so it cannot repost messages.
...
fight IDs plus a normalized Discord outcome and bounded counts as a typed receipt. The coordinator
retains that receipt for persistent history and
